"""
CSV → Houses DB import pipeline.
Usage:  python -m app.dataProcessing.import_csv_to_db  <path_to_csv>
"""

# ── 1. CONSTANTS ─────────────────────────────────────────────────────────────

UMASS_COORDS = (42.413939, -72.528618)   # UMass Amherst lat/lon

CSV_IMPORT_BOT_UID = "csv-import-bot"

# Null-like strings in raw CSV cells
_EMPTY_STRINGS = {"", "nan", "none", "n/a", "null", "na", "not available", "n/a.", "--", "-"}

# Amenity keyword sets (lowercase, stripped)
KITCHEN_KEYWORDS = {
    "kitchen", "kitchenette", "full kitchen", "eat-in kitchen",
    "eat in kitchen", "galley kitchen", "open kitchen",
    "cooking", "stove", "oven", "range", "microwave",
    "refrigerator", "fridge", "dishwasher",
}

WASHER_KEYWORDS = {
    "washer", "washer/dryer", "washer and dryer", "washer & dryer",
    "laundry", "in-unit laundry", "in unit laundry",
    "on-site laundry", "on site laundry", "laundry facility",
    "laundry room", "laundry hookup", "laundry hook-up",
    "washing machine", "dryer", "w/d", "w/d hookup",
}

PARKING_KEYWORDS = {
    "parking", "parking space", "parking spot", "parking included",
    "off-street parking", "off street parking", "on-street parking",
    "assigned parking", "covered parking", "underground parking",
    "garage", "attached garage", "detached garage", "carport",
    "driveway", "private driveway",
}

# Minimum non-null key fields required to keep a row
# (city, province, street, distance_to_university)
MIN_NULL_THRESHOLD = 2   # skip row if null count >= this value


# ── 2. PURE PARSERS ──────────────────────────────────────────────────────────

import re
import math
from typing import Optional


def _clean(v) -> Optional[str]:
    """Normalise a raw CSV cell to str or None."""
    if v is None:
        return None
    s = str(v).strip()
    return None if s.lower() in _EMPTY_STRINGS else s


def _haversine(lat: float, lon: float) -> float:
    """Return distance in km between (lat, lon) and UMass Amherst."""
    R = 6371.0
    lat1, lon1 = math.radians(UMASS_COORDS[0]), math.radians(UMASS_COORDS[1])
    lat2, lon2 = math.radians(lat), math.radians(lon)
    dlat = lat2 - lat1
    dlon = lon2 - lon1
    a = math.sin(dlat / 2) ** 2 + math.cos(lat1) * math.cos(lat2) * math.sin(dlon / 2) ** 2
    return R * 2 * math.asin(math.sqrt(a))


# Matches patterns like "123", "123A", "123-125", "Unit 4B", "#7"
_HOUSE_NUMBER_RE = re.compile(
    r"""^
    (?:
        (?:unit|apt|apartment|suite|ste|#)\s*[\w-]+  # Unit 4B / Apt 2 / #7
        |
        \d+[\w-]*                                     # 123 / 123A / 123-125
    )
    """,
    re.VERBOSE | re.IGNORECASE,
)


def _parse_address(raw: str) -> tuple[Optional[str], Optional[str]]:
    """
    Split a raw address string into (house_number, street).
    Returns (None, None) if raw is empty/null.
    """
    raw = raw.strip()
    if not raw:
        return None, None

    m = _HOUSE_NUMBER_RE.match(raw)
    if m:
        house_number = m.group(0).strip()
        street = raw[m.end():].strip().lstrip(",").strip()
        return house_number or None, street or None

    # No recognisable house number — treat entire string as street
    return None, raw or None


def _parse_amenities(raw: Optional[str]) -> tuple[bool, bool, bool]:
    """
    Return (has_kitchen, has_washer, has_parking) by keyword matching.
    Matching is case-insensitive on the full amenities string.
    """
    if not raw:
        return False, False, False
    text = raw.lower()
    has_kitchen = any(kw in text for kw in KITCHEN_KEYWORDS)
    has_washer  = any(kw in text for kw in WASHER_KEYWORDS)
    has_parking = any(kw in text for kw in PARKING_KEYWORDS)
    return has_kitchen, has_washer, has_parking
